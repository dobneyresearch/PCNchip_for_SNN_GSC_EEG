...
**The paper.** `paper/main_stage3_GSC_EEG_v1.pdf` and its LaTeX/Markdown sources are a scholarly
work, not software, and a software licence is the wrong instrument for them. They are **© Saul
Dobney, all rights reserved**, except that quotation and citation under normal academic convention
and applicable fair use or fair dealing are unaffected. If the paper is posted to a preprint server
it will carry whatever licence that server requires, and that grant governs the copy hosted there.

**The Sky130A base process.** The temporal datapath in `rtl_temporal/` is synthesisable Verilog
written for the open SkyWater Sky130A process used in Parts I and II. The Sky130A PDK is licensed
separately under Apache 2.0 by its own authors, is not included here, and is not covered by these
licences.

**The datasets.** Neither benchmark dataset is included, and each is distributed by its own authors
under its own terms: **Google Speech Commands** (Warden, 2018), used here under the NeuroBench
temporal protocol (Yik et al., 2024); and the **THOR EEG motor-imagery** challenge data. Obtain each
from its own source under its own licence.

**Third-party tooling.** `iverilog`, NumPy, PyTorch, pandoc and TeX are all separately licensed by
