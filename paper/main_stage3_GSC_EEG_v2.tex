...
\begin{abstract}
We have shown a design of analog PCN chip that learns supervised tasks
\emph{forwards-only} --- no backward pass and no host in the training
loop --- on static images. Here we extend the same forwards-only,
hardware-constrained approach to handle spiking temporal data and show
credit assignment across time and demonstrate it on \emph{two} very
...
Once again we find a role for a \emph{leaky accumulator},
\(x_t = \lambda x_{t-1} + u_t\), as a primary hardware element, one that
can assign temporal credit through a short \emph{local} eligibility
window (a forward-mode approximation to BPTT, not a backward pass), to
consolidate weights and learning. Across both tasks the design reaches
about \textbf{94\% of the full-BPTT (Adam) upper bound} on an identical
network: \(0.832\) vs \(0.881\) on speech (\(0.866\) once the buffer's
delay is made learnable, \({\sim}98\%\)), and \(0.668\) vs \(0.709\) on
EEG --- each with a shallow, few-step local buffer standing in for a
\(200\)--\(250\)-step backward unroll. This shows the same
forwards-only, asynchronous, robust design can serve for temporal and
static data based learning.

In particular, the same primitive generalises across two modalities
without task-specific design, and the single setting that differs
between them --- a reliability-graded versus a \(1\)-bit-sign weight
write --- depends only on whether temporal credit is a noisy
per-timestep sum, which the EEG task independently confirms. Along the
way we see the use of multiple leaky accumulators in the design,
pointing to a potential central and repeatable role in analog-based
learning.

The temporal learning mechanism itself is a set of bolt-on modules to
the original static cell design, realised in synthesisable RTL verified
\emph{bit-exact} against a fixed-point model, including a two-layer
datapath that trains both layers forwards-only; all results are
pre-silicon.
\end{abstract}
...
no global clock \cite{dobney2026analog, dobney2026forwards}. We
deliberately fix three design constraints --- signal paths one-way
...
On static image classification (Part II) the rule reached
backpropagation level accuracy given an identical network topology.

We now look at whether the design can assign credit \emph{across time},
capturing and learning from spiking signals that are temporal rather
than spatial. The standard approach to spiking data is
backpropagation-through-time (BPTT), which uses the whole time sequence
and runs the backward pass through every step to calculate the weights.
However, the global, backward operations are not possible in a
forwards-only hardware design. In particular, it is also physically
acausal --- it computes a correction at time \(t\) from information at
times \(t' > t\), which no forward-running substrate can supply.

Here we extend the forwards-only, hardware-constrained approach to
temporal credit assignment, show that it learns, and measure it against
BPTT retaining our key design constraints from Part II --- one-way
signal paths, tolerance of coarse and mismatched devices, no global
clock, a small \emph{local} memory per cell --- and search for
hardware-feasible mechanisms that fit them, judged against
backpropagation aiming to see how close a realisable, forwards-only
system can get to BPTT.

In solving each of the key sub-problems for this design --- how to
...
\textbf{leaky accumulator}, described as a sigma-delta design in Part
II,
...
The intuition of the leaky accumulator is that it gathers, smooths and
filters signals and, in a temporal setting, retains an `echo' of
previous signals. Small data updates are added to the accumulator so
incoming signals that are noisy and random cancel each other out; the
leak acts as a decay; and threshold trigger means weights are only
updated when the signal grows to become meaningful. In hardware terms
this is easily constructed as a capacitor design, and is what we refer
to as a `leaky jug'.

The strongest evidence that leaky accumulators are a core property of
the mechanism, and not of one dataset, is that it holds across two
temporal signals that could hardly be more different. \textbf{Google
Speech Commands} (GSC) is delta-modulated audio, sparse and event-like,
decoded by a deep spiking stack. \textbf{THOR EEG motor imagery} is
continuous band-limited brain activity whose class information lives in
signal \emph{power}, decoded by a shallow one. The same cell, the same
credit mechanism, and the same forwards-only discipline handle both.
Crucially, we did not re-engineer the architecture per task: the one
setting that differs between the two is not a tuned hyperparameter but a
\emph{consequence of a tested law} (Sec. 6). This is the
\textbf{robustness} invariant --- a generalist architecture rather than
a task-specific one --- made concrete.
...
A temporal task breaks that assumption such that the forward pass
becomes a spiking recurrence over \(T\) timesteps, and the error
observed at the read-out must be attributed to weights according to
\emph{when} their activity mattered. BPTT does this by holding the
entire unrolled history and propagating the error backward through it.
Three properties make that unacceptable here: it is a backward pass
(forbidden on our analog path by construction); it buffers the whole
sequence (a \(200\)--\(250\)-deep store per unit); and it is acausal in
time. The question of this paper is whether a \emph{forward, local,
shallow} mechanism can recover the credit that BPTT computes. Everything
else in Part II --- the jug, the fold, the transpose-at-source, the
...
forwards-only rule removes. Our aim is to approach BPTT's accuracy
...
constant \(1/(1-\lambda)\). The whole design uses multiple variations of
this cell with different leaks and different inputs, sometimes read
through a threshold, sometimes through a bounded graded write. Read one
way it is a rolling sum that smooths a noisy stream; read another it
triggers a discrete event when its total crosses a level. We take the
roles in turn.
...
past activity to reward. BPTT does this by propagating the error
...
realised by a hardware-credible \textbf{\(n\)-deep buffer of timesteps,
held at the cell/layer level}. The buffer allows us to mimic the
forwards-in-time calculations of BPTT, acting as a time-delay window
which makes \(\bm\delta_{\ell,t}\) available \(n\) steps late, a fixed
learning delay rather than a look-ahead. The weights \(w_k\) over the
window are the \textbf{tap weights} (Sec. 4.3): setting them all to
\(1\) recovers a uniform box; making them learnable lets the buffer
discover which delays carry credit. This adjoint does two jobs. It is
\textbf{routed to the layer below} by the transpose of the layer's
\emph{own} weights,
...
\eqref{eq:fold} (i.e.~\(+1\) or \(-1\)) is the base rule, and on tasks
whose credit is an \emph{aggregate} it is all that is needed --- as we
will see, it is what the EEG task uses. On the GSC temporal task it is
not enough, because the per-timestep credit in \eqref{eq:credit} is a
\emph{noisy sum}: individual timesteps disagree, and a bare sign
discards how \emph{consistent} the evidence was. There we replace the
sign with a reliability-graded step --- itself two more leaky
accumulators, of the credit's mean and of its energy:
...
keep \emph{only} when credit is a noisy per-timestep sum.

\hypertarget{sec:readout}{%
\subsection{On-chip read-out: the second half, also
forwards-only}\label{sec:readout}}

Credit assignment in the hidden layers is the first half of the
learning. In addition, the classifier read-out is the second half. On
the static chip the classifier was fit off-line by a host least-squares
step. On these temporal tasks we also train the classifier on-chip by
its own gradient. Consequently, the read-out classifier sees a feature
vector \(\bm f\) --- for GSC a \emph{leaky-integrated} one,
\eqref{eq:prim} once more; for EEG an aggregate spike \emph{rate} ---
and updates by the output error:
...
implements, the temporal capability can be considered as a set of
\emph{bolt-on modules}, not a redesign. Each can be included or omitted
at the cell/chip/layer level per task, and that separability is what
keeps the design modular and the key arithmetic at the neuron level,
where it scales.
...
\emph{tiered} hardware story follows directly from the testing in Sec.
...
The modular hardware argument has also been tested by implementing the
temporal datapath in synthesisable register-transfer-level (RTL)
Verilog, with each block verified \textbf{bit-exact} against a
fixed-point reference model --- the same double-build discipline Part II
used for the static jug, on the same open \(130\) nm base and simulation
harness.
...
This turns ``hardware-feasible'' from an argument into a measured result
with datapaths verified at the block, tile, and two-layer level, though
not yet as a full swept array; resource \emph{sharing} --- one
comparator and one square-root/divide time-multiplexed across the array
--- is an area optimisation on top of the proven datapath.
Transistor-level SPICE of the new cells and place-and-route remain, as
in Part II, the pre-silicon steps beyond this study.
...
The design was tested using the \emph{same} architecture on two temporal
signals of opposite character, without per-task finessing of the rule
parameters.
...
\textbf{The learnable tap weights / buffer reach.} On GSC the learnable
tap weights are worth \(+0.032\) (uniform \(0.834 \to\) tapped
\(0.866\)); on EEG the window depth is worth \(+0.030\) (\(K8\to K16\)).
On both, buffer \emph{depth} is otherwise flat between \(6\) and \(12\)
uniform steps, and the uniform box matches the exact decaying adjoint
over \(3\) seeds (\(+0.004\), not significant) --- so the hardware
implements the simpler box and spends its parameters on the taps, which
is where reach is actually bought.
...
\item
  ...
  parameter finessing --- the same rule, applied, tells you which module
  to enable.
\item
...
accept. A maths-perfect BPTT result is not necessarily physically
reachable on a forward, local, coarse substrate, and chasing the last
points by adding a backward channel or unbounded per-synapse memory
would forfeit the very properties the design exists to provide. A robust
generalist that reaches \({\sim}94\)--\(98\%\) of the backpropagation
ceiling on two unlike temporal tasks, forwards-only and with a few
hundred bits of local buffer, is the useful object. We assume Nature
does not compute a perfect gradient either; it computes a good-enough
one, locally and forward, and so do we.
